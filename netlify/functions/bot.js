...
const typeNames = {
  ...
  s:'Картонная коробка (самосборная)', x:'Картонная коробка'
};

// Базовые цены для multi-pack режима (минимальная цена в категории)
const basicPackPrices = { v:3, z:15, b:18, u:15, k:15, s:15, x:22 };
...
const PACK_SUBSTEPS = { v:'pvpp', z:'pzip', b:'pbopp', u:'pbubble', k:'pcour', s:'psbox', x:'pbox' };
...
function backFrom(s, cur) {
  ...
  if (['pvpp','pzip','pbopp','pbubble','pcour','psbox','pbox'].includes(cur)) {
