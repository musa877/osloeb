// Cloudflare Pages Function: Telegram-бот GoPack.
// Маршрут: /bot — туда Telegram шлёт webhook.
// Хранилище состояния: Cloudflare KV (биндинг env.BOT_PENDING).
// Переменные окружения: BOT_TOKEN, CHAT_ID (опц. MANAGER_CHAT_ID).
...
function togglePack(current, ch) {
  ...
  if (cur.includes(ch)) return cur.replace(ch, '') || null;
  return cur + ch;
}
...
async function setPending(env, chatId, data) {
  try {
    await env.BOT_PENDING.put('chat:' + chatId, JSON.stringify(data), { expirationTtl: 1800 });
  } catch (e) {}
}
async function getPending(env, chatId) {
  try { return await env.BOT_PENDING.get('chat:' + chatId, 'json'); }
  catch (e) { return null; }
}
  ...
  switch (s.step) {
    ...
    case 'pm': {
      ...
      if (current.length > 0) {
        ...
        rows.push([{ text: `✓ Готово (упаковка: +${sum} ₽/шт)`, callback_data: enc(s, 'd') }]);
      }
      ...
      return {
        text:
          ...
          '<b>Несколько типов упаковки</b>\nКлик по варианту — добавить/убрать.\n\n' +
          `<i>Выбрано: ${sel}</i>`,
        keyboard: rows
      };
    }
...
async function sendPrompt(env, chatId, text, placeholder) {
  await tg(env, 'sendMessage', {
    chat_id: chatId, text, parse_mode: 'HTML',
    reply_markup: { force_reply: true, input_field_placeholder: placeholder || '' }
  });
}
async function sendPhonePrompt(env, chatId) {
  await tg(env, 'sendMessage', {
...
async function removeReplyKeyboard(env, chatId, text) {
  await tg(env, 'sendMessage', {
...
export async function onRequestPost({ request, env }) {
  if (!env.BOT_TOKEN || !(env.MANAGER_CHAT_ID || env.CHAT_ID)) {
    return new Response('not configured', { status: 200 });
  }

  let upd;
  try { upd = await request.json(); }
  ...
  try {
    ...
    if (upd.message && upd.message.text) {
      ...
      if (cmd === '/tungtungsahur') {
        await tg(env, 'sendMessage', { chat_id: chatId, text: 'привет любимая ❤️' });
        return ok();
      }
      if (cmd === '/start' || cmd === '/calc') {
        await clearPending(env, chatId);
        await sendStep(env, chatId, { step: 'm' });
        return ok();
      }
      if (cmd === '/help') {
        await tg(env, 'sendMessage', {
      ...
      if (cmd === '/debug') {
        ...
        try {
          await env.BOT_PENDING.put('chat:' + chatId + ':' + testKey, JSON.stringify({hello:'world'}), { expirationTtl: 60 });
          report += '✅ put ok\n';
          const got = await env.BOT_PENDING.get('chat:' + chatId + ':' + testKey, 'json');
          report += got && got.hello === 'world' ? '✅ get ok\n' : '❌ get вернул не то\n';
          await env.BOT_PENDING.delete('chat:' + chatId + ':' + testKey).catch(() => {});
          report += '✅ delete ok\n';
        } catch (e) { report += '❌ Ошибка KV: ' + e.message + '\n'; }
        const pending = await getPending(env, chatId);
        report += '\n📦 pending: ' + (pending ? JSON.stringify(pending) : 'нет');
        await tg(env, 'sendMessage', { chat_id: chatId, text: report, parse_mode: 'HTML' });
        return ok();
      }
      ...
      if (pending && (lc === 'назад' || lc === 'back')) {
        if (pending.step === 'qresp') {
          await clearPending(env, chatId);
          await sendStep(env, chatId, { ...pending.s, step: 'q' });
        } else if (pending.step === 'phoneresp') {
          await clearPending(env, chatId);
          await removeReplyKeyboard(env, chatId, '⬅️ Возврат к расчёту');
          await sendStep(env, chatId, { ...pending.s, step: 'done' });
        } else if (pending.step === 'commentresp') {
          await setPending(env, chatId, { step: 'phoneresp', s: { ...pending.s, phone: null } });
          await sendPhonePrompt(env, chatId);
        }
        return ok();
      }
      ...
      if (pending) {
        if (pending.step === 'qresp') {
          ...
          await clearPending(env, chatId);
          await sendStep(env, chatId, newS);
          return ok();
        }
        ...
        if (pending.step === 'commentresp') {
          ...
          await clearPending(env, chatId);
          await sendToManager(env, chatId, pending.s, comment, upd.message.from);
          return ok();
        }
      }
    ...
    // ────── INLINE-КНОПКИ ──────
      ...
      if (data.startsWith('S|')) {
        ...
        if (s.step === 'qask') {
          await setPending(env, chatId, { step: 'qresp', s });
          await sendPrompt(env, chatId,
        ...
        if (s.step === 'phoneask' || s.step === 'send') {
          await setPending(env, chatId, { step: 'phoneresp', s });
          await sendPhonePrompt(env, chatId);
          return ok();
        }
